            ...
            .insert(
                {
                    ...
                    "stage": stage,
                    "payload": payload or {},
                    # Migration 0008 added a NOT-NULL `source` discriminator
                    # (default 'worker') so the dashboard / debugging tools
                    # can tell worker emissions apart from the Hermes
                    # hook/task emitters. Set it explicitly so this stays
                    # correct even if the column default ever changes.
                    "source": "worker",
                }
            )
            .execute()
