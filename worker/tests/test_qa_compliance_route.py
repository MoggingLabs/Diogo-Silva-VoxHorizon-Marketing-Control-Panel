...
def _seed_video_creative(
    fake, *, script_outline: dict, creative_id: str = VIDEO_CREATIVE_ID
) -> None:
    """Seed a ``video_creatives`` row (no matching ``creatives`` row).

    ``_fetch_creative`` queries ``creatives`` first (miss), then
    ``video_creatives`` — so a video id resolves to its spoken script.
    """
    fake.seed(
        "video_creatives",
...
async def test_compliance_run_video_spoken_claim_blocks(
    ...
    """VID-10: a personal-attribute claim SPOKEN in the voiceover fails the gate.
