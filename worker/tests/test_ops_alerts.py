...
async def test_observability_tick_delivers_alert_on_dead_letters(
    ...
    sb = _patch_sb
    # Silent-failure PR-6: the outbox "dead" bucket = work_item kind=outbox_*,
    # status=timed_out (the watchdog's dead-letter terminal).
    sb.seed(
        "work_item",
        [
            {"kind": "outbox_meta_record_launch", "status": "timed_out"},
