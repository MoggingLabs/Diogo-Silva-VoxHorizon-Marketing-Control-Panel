...
describe("GET /api/creatives/video/[id]/iterations", () => {
  it("400s when id is empty", async () => {
    const res = await GET(new Request("http://x") as never, ctx(""));
    expect(res.status).toBe(400);
  });

  it("returns iterations for the video creative", async () => {
    result = { data: [{ id: "i1" }], error: null };
    const res = await GET(new Request("http://x") as never, ctx("v1"));
    expect(await res.json()).toEqual({ iterations: [{ id: "i1" }] });
    expect(from).toHaveBeenCalledWith("video_iterations");
  });
