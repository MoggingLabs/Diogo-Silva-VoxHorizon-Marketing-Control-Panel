...
/**
 * POST /api/creatives/video/:id/restore
 *
 * Un-archive a soft-deleted video creative: clears `deleted_at` so it reappears
 * in the active grid. The mirror of `DELETE /api/creatives/video/:id`.
 *
 ...
 * non-fatal `video_creative_restored` audit event on success.
 */
export async function POST(_req: NextRequest, ctx: RouteContext) {
  const { id } = await ctx.params;
  const supabase = createAdminClient();

  const result = await restore<VideoCreative>(supabase, "video_creatives", id);

  switch (result.kind) {
    case "ok":
      await emitEvent(supabase, {
        kind: eventKind("video_creative", "restored"),
