...
- **Network boundary: Caddy + UFW.** The VPS runs Caddy as the only externally-exposed app service (`:80` / `:443` / `:443-udp`). Caddy fronts only the dashboard at `dashboard.voxhorizon.com`; the worker container has no public port binding. UFW denies all inbound except 22/80/443, and an `iptables` `DOCKER-USER` rule (matched by `conntrack --ctorigdstport`) drops the Hermes ttyd port range (32787–32796) that Docker would otherwise publish to `0.0.0.0`. DNS is currently GoDaddy (no Cloudflare proxy yet) — a Cloudflare migration for SSL Full-strict + WAF + Access SSO is a planned v1.5 hardening step.
- **App boundary: Caddy HTTP Basic Auth (v1).** The dashboard sits behind a single-operator Basic Auth gate at the Caddy edge (bcrypt cost 14, `CADDY_BASIC_AUTH_USER` / `CADDY_BASIC_AUTH_HASH`). Every request to the Next.js app is authenticated at that layer; an unset hash fails Caddy validation (fail-closed). Note: the gate covers the _dashboard_ origin only — the Supabase REST API is a separate public endpoint protected by the database boundary below, NOT by this gate. SSO (Cloudflare Access or NextAuth) replaces Basic Auth when the team grows past one operator.
...
- **Database boundary: RLS lockdown + service role.** The Supabase REST/Realtime API is **public** (a separate endpoint from the dashboard — the Caddy Basic Auth gate does NOT protect it), and the anon key ships in the browser bundle, so the database must defend itself. Migration `0010` revoked all write/delete/truncate grants from the `anon` + `authenticated` roles (SELECT-only) — closing the ability for any anon-key holder to forge or destroy data. Migration `0011` (Phase 2) enables RLS deny-all on every public table so anon has no read access either; `lib/supabase/server.ts` switches to the service-role key (`SUPABASE_SECRET_KEY` → `service_role` role, which has `BYPASSRLS`) for server-component reads, and client-side Realtime moves to a server-side SSE relay so the browser never holds a Supabase connection. After Phase 2 the anon key has zero useful access; all DB access flows through the service role behind the Caddy gate. **Note:** "service role" here means the `service_role` _Postgres role_ (alive, `rolbypassrls=true`) — not the legacy `service_role` _JWT key_ (deprecated; we use the `sb_secret_*` key format, which authenticates as that role).
...
| `SUPABASE_SECRET_KEY`                   | `/opt/voxhorizon/.env` (runtime, both containers) + `.env.local` + `worker/.env` + Vault                          | Server-side admin client; bypasses RLS                                                                     | Supabase dashboard                                                                                                                                                                                                         | Quarterly + on suspected leak                                      |
...
| `CADDY_BASIC_AUTH_USER`                 | `/opt/voxhorizon/.env` (runtime, caddy container) + Vault (`dashboard-credentials.env`)                           | Username for the dashboard's edge HTTP Basic Auth gate                                                     | Edit `/opt/voxhorizon/.env`; restart caddy                                                                                                                                                                                 | On operator change                                                 |
| `CADDY_BASIC_AUTH_HASH`                 | `/opt/voxhorizon/.env` (runtime, caddy container) + Vault (`dashboard-credentials.env` keeps the plaintext)       | bcrypt hash (cost 14) the Caddy edge checks Basic Auth against. See the **bcrypt `$$` escape** note below. | Regenerate: `python -c "import bcrypt,secrets; pw=secrets.token_urlsafe(24); print(pw); print(bcrypt.hashpw(pw.encode(), bcrypt.gensalt(14)).decode())"` → escape `$`→`$$` → update `/opt/voxhorizon/.env` → restart caddy | Quarterly + on suspected leak                                      |
...
> **⚠️ bcrypt `$$` escape — read before rotating `CADDY_BASIC_AUTH_HASH`.** A bcrypt hash is full of `$` characters (`$2b$14$<salt><hash>`). Docker Compose runs **one pass of variable interpolation** over `env_file` values, so every `$<name>` segment inside the raw hash (e.g. `$2b`, or `$<saltchars>`) gets treated as a variable reference and substituted to empty — the hash arrives at the Caddy container truncated and **every Basic Auth attempt fails with 401, even with the correct password.** The fix: **double every `$` to `$$`** in the `/opt/voxhorizon/.env` value. Compose collapses `$$`→`$` during interpolation, so Caddy receives the intact 60-char hash. Verify after a rotation with: `docker exec voxhorizon-caddy-1 sh -lc 'echo ${#CADDY_BASIC_AUTH_HASH}'` — it must print `60`, not `39`. The plaintext password is kept (unescaped) in `~/.config/voxhorizon/dashboard-credentials.env` on the dev box.
...
| `SUPABASE_SECRET_KEY`                       | web, worker                                                                                   | Supabase Secret API key (new format `sb_secret_*` — opaque, rotatable independently; legacy service_role JWT also accepted). Bypasses RLS. Server-only. **Both containers read it.**                 | Supabase dashboard → Project Settings → API Keys → "Publishable and Secret API Keys" → Rotate Secret. Apply to `/opt/voxhorizon/.env`; roll both containers. |
...
High-urgency approvals fan out to Slack (`chat.postMessage` via the `Ekko` bot in the `voxhorizon-internal` workspace) as of the 2026-05-18 pivot. The previous Resend email path is dormant code (route + react-email template preserved in `app/api/internal/approval-email/route.ts` and `lib/emails/HighUrgencyApprovalEmail.tsx`); `RESEND_API_KEY` is no longer required for v1 but remains inventoried in case the operator reverses the pivot.

| Var                            | Consumed by                                  | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                        | Rotate at                                                                             |
| ------------------------------ | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- |
| `SLACK_BOT_TOKEN`              | worker                                       | Bot token (`xoxb-...`) for the `Ekko` bot in workspace `voxhorizon-internal` (team `T075KUK0U3F`, bot `B0AKKBG3KMF`). Used by `worker.approval_notifications.fan_out` to post high-urgency approvals via `chat.postMessage`. **Source at deploy time from `/docker/hermes-shared/config/secrets.json` (field `EKKO_SLACK_BOT_TOKEN`)** — the deploy step copies the value into the worker's runtime env. Never paste into git. | Slack workspace admin → Apps → Ekko → OAuth & Permissions → Reinstall.                |
| `SLACK_APPROVAL_CHANNEL_ID`    | worker                                       | Static Slack channel ID for the destination channel (`#mkt-dept-updates`, ID `C0B43582YJF`). The bot must be a member. Not a secret — listed here for inventory.                                                                                                                                                                                                                                                               | Edit `/opt/voxhorizon/.env` only when routing target changes.                         |
| `DASHBOARD_BASE_URL`           | worker                                       | Base URL embedded in the Slack "Open in dashboard" CTA. Defaults to `https://dashboard.voxhorizon.com` in code if unset.                                                                                                                                                                                                                                                                                                       | Edit when the public dashboard URL changes.                                           |
| `RESEND_API_KEY` (dormant)     | web (dormant)                                | Server-side transactional email. **No longer required for v1** — kept for the dormant `/api/internal/approval-email` route. Set only if reviving the email path.                                                                                                                                                                                                                                                               | Resend dashboard.                                                                     |
...
| `SLACK_BOT_TOKEN`                 | worker       | Slack bot token used by `worker.approval_notifications.fan_out` to post high-urgency approvals to `#mkt-dept-updates`. **Sourced at deploy time from `/docker/hermes-shared/config/secrets.json` (key `EKKO_SLACK_BOT_TOKEN`).** Never paste in git. | Slack admin → reinstall Ekko app; update `/docker/hermes-shared/config/secrets.json` AND `/opt/voxhorizon/.env`; restart worker.  |
| `SLACK_APPROVAL_CHANNEL_ID`       | worker       | Static Slack channel ID for the high-urgency approvals channel (`C0B43582YJF` = `#mkt-dept-updates`). Not secret.                                                                                                                                    | Edit when routing target changes.                                                                                                 |
| `DASHBOARD_BASE_URL`              | worker       | Base URL used in the Slack "Open in dashboard" CTA button.                                                                                                                                                                                           | Edit when the public dashboard URL changes.                                                                                       |
| `INTERNAL_API_TOKEN` (dormant)    | worker, web  | Bearer the worker carried when calling Next.js `/api/internal/*` (e.g. the high-urgency email render endpoint). **Unused after 2026-05-18 Slack pivot.** Listed for inventory; required only if the email path is revived.                           | Same procedure.                                                                                                                   |
| `INTERNAL_API_BASE_URL` (dormant) | worker       | Base URL the worker used to reach the web container for internal calls. **Unused after 2026-05-18 Slack pivot.**                                                                                                                                     | Only change if `web` is renamed in `docker-compose.yml`.                                                                          |
| `OPERATOR_EMAIL` (dormant)        | web          | Resend recipient for high-urgency approval emails. **Unused after 2026-05-18 Slack pivot** — the Slack channel is now the high-urgency surface. Required only if reviving the email path.                                                            | Edit when the operator's email changes.                                                                                           |
...
| `SUPABASE_SECRET_KEY`            | Ekko (`dashboard-publish` skill helpers) | Service-role key the skill helpers use. **Same value as the worker's** — Supabase has one canonical service-role key. | Rotate at Supabase dashboard; update `/opt/voxhorizon/.env` AND `/opt/data/.env`; restart both. |
...
4. **Restart the container(s) that consume it.** If only the worker reads the var, restart just `worker`; same for web-only. For shared values (e.g. `WORKER_SHARED_SECRET`, `SUPABASE_SECRET_KEY`), restart both:
...
- `SUPABASE_SECRET_KEY` — the admin key both sides use to talk to Postgres.

Because both containers read the same `/opt/voxhorizon/.env` and `docker compose up -d` rolls them sequentially, there is a brief (~5–10s) window where one container has the new value and the other still has the old. For `WORKER_SHARED_SECRET` that means transient 401s; for `SUPABASE_SECRET_KEY` it means transient 5xx from one side. Both windows are short enough that for a single-operator system the simpler procedure is "rotate during a known idle moment, accept the brief blip." If a zero-downtime swap is required, use this rolling pattern:
...
The same pattern applies to `SUPABASE_SECRET_KEY`: stage `SUPABASE_SECRET_KEY_NEXT`, teach the relevant client construction to fall back, promote, clean up, invalidate the old key.
